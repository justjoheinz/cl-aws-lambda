(defpackage cl-aws-lambda/runtime-interface
  (:use :cl :alexandria)
  (:import-from :cl-aws-lambda/conditions #:runtime-error #:message-of)
...
(defvar *aws-lambda-runtime-api* nil "The hostname/port of the lambda runtime.")
...
(defvar +lisp-lambda-runtime-agent+
  (load-time-value
   (format nil "cl-aws-lambda (~a ~a)"
           (uiop/common-lisp:lisp-implementation-type)
...
(defun make-runtime-url (&rest path-components)
  ...
  (let ((*aws-lambda-runtime-api* (uiop:getenv "AWS_LAMBDA_RUNTIME_API")))
    (assert *aws-lambda-runtime-api* ()
      "Tried to assemble a runtime url, but *aws-lambda-runtime-api* was nil.")
    (format nil "http://~a/~a/~{~a~}" *aws-lambda-runtime-api* *api-version*
            path-components)))
...
(defun next-invocation ()
  ...
  (multiple-value-bind (body status headers)
      (dex:get (make-runtime-url "runtime/invocation/next")
    ...
    (assert (= status 200) ()
      "The runtime interface returned a value of ~d, the body of the response was: ~S"
      status body)
...
(defmacro do-events ((event) &body body)
  (with-gensyms (context)
    `(loop for (,event ,context) = (multiple-value-list (next-invocation))
           do (let ((*context* ,context))
                ...
                (when (sampled-p *context*)
                  (setf (uiop:getenv "_X_AMZN_TRACE_ID")
                          (trace-id-of *context*)))
                ,@body))))
...
(defun invocation-response (content)
  ...
  (declare (optimize (speed 3) (space 3) (safety 1) (compilation-speed 0)))
  (assert *context* ()
    "Tried to report an invocation error but *context* was unbound.")
  (log:debug "Responding to event with ~S." content)
  (multiple-value-bind (body status headers)
      (dex:post
        (make-runtime-url "runtime/invocation/" (request-id-of *context*)
                          "/response")
        :headers
  ...
  (multiple-value-bind (body status headers)
      (dex:post
        ...
        :content (jojo:with-output-to-string*
                   (jojo:with-object
                     (jojo:write-key-value "errorMessage"
                                           (or (message-of error) ""))
                     (jojo:write-key-value "errorType"
...
(defun initialization-error (error)
  ;; (declare (type runtime-error error))
  (log:error "Initialization Error [~a]: ~a" (class-name (class-of error))
             error)
  (multiple-value-bind (body status headers)
