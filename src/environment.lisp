(defpackage cl-aws-lambda/environment
  (:use :cl :alexandria)
  (:import-from #:cl-aws-lambda/conditions #:environment-error)
...
(defvar *lambda-task-root* nil "The function that contains the function code.")
  ...
  (flet ((check-env (name)
           (unless (uiop:getenvp name)
             (error 'environment-error
                    :message (format nil
                                     "Environment variable ~S was not set during initialization."
                                     name)))))
    (check-env "_HANDLER")
