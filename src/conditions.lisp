...
(define-condition runtime-error (error)
  ...
  (:report
   (lambda (c s)
     (format s "Condition ~S was signalled~@[ ~a~]" (class-name (class-of c))
             (when (slot-boundp c 'message)
               (slot-value c 'message)))))
  (:documentation
   "The base class for all errors that can be the result of a lambda invocation."))

(define-condition environment-error (runtime-error)
  ()
  (:documentation
   "An error that indicates something is wrong with the environment variables that set up the runtime."))

(define-condition uncaught-error (runtime-error)
  ()
  (:documentation
   "An error that the user didn't handle explicitly was thrown and the runtime caught it."))

(defun handle-uncaught-initialization-error (e)
  (make-instance 'uncaught-error
                 :message (format nil
                                  "Uncaught error signaled during initialization:~%  ~a~%~%"
                                  e)))

(defun handle-uncaught-invocation-error (e)
  (make-instance 'uncaught-error
                 :message (format nil
                                  "Uncaught error signaled during invocation:~%---~%  ~a~%---~%"
                                  e)))